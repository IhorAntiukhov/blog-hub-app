...
function MultipleSelect({ value, onChange, title, options }) {
  ...
  const checkboxClass = classNames('relative', 'text-xl', 'pl-10', 'cursor-pointer',
  ...
  const multipleSelectOptionsClass = classNames('absolute', 'flex', 'flex-col', 'w-full', 'bg-neutral-3', 'rounded-lg',
  ...
  return (
    <div className="relative space-y-1" ref={multipleSelectEl}>
      <div className={multipleSelectClass}
        onClick={() => {
          setIsOpen((isOpen === 2) ? 1 : 2);
        }}>
