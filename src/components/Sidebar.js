  ...
  return (
    <nav className="z-20 flex flex-col justify-center space-y-8 h-full border-r-[3px] border-neutral-3 bg-neutral-2">
      ...
      <SidebarLink href="/subscriptions" title="For you" selected={currentPath === '/subscriptions'}>
        <ReactIcon src={<IoMdNotifications className="w-12 h-12" />} color={defineIconColor('/subscriptions')} />
      </SidebarLink>

      <SidebarLink href="/marked" title="Marked" selected={currentPath === '/marked'}>
        <ReactIcon src={<BiSolidBookmark className="w-12 h-12" />} color={defineIconColor('/marked')} />
      </SidebarLink>

      <SidebarLink href="/profile" title="Profile" selected={currentPath === '/profile'}>
        <ReactIcon src={<BiSolidUser className="w-12 h-12" />} color={defineIconColor('/profile')} />
      </SidebarLink>
    </nav>
  );
}
